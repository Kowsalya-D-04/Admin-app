...
const LoadRequests = () => {
  ...
  // };
  const assignDriverToLoad = async () => {
  const customerLoadId = selectedLoad?.id ?? selectedLoad?.requestId;
  const driverLoadRequestId = selectedDriver?.id ?? selectedDriver?.requestId;

  if (!customerLoadId || !driverLoadRequestId) {
    setError('Please select customer load and driver request');
    return;
  }
  ...
  try {
    const res = await axios.post(
      `${API_URL}/assign/customer-to-driver`,
      payload
    );
